...
module Lemmas where
  ...
  ⊢nv-embed-naturality unitNV = 
      ⋆ 
    v∎
  ⊢nv-embed-naturality zeroNV = 
      zero
    v∎
  ⊢nv-embed-naturality oneNV = 
      one
    v∎
  ...
  rename-subst-lem-v {Γ} {Γ'} {Γ''} {unit} {s} {f} ⋆ = 
      ⋆
    ∎
  rename-subst-lem-v {Γ} {Γ'} {Γ''} {bit} {s} {f} zero = 
      zero
    ∎
  rename-subst-lem-v {Γ} {Γ'} {Γ''} {bit} {s} {f} one = 
      one
    ∎
  ...
  rename-subst-lem-p {Γ} {Γ'} {Γ''} {σ} {s} {f} (or t u) = 
    ...
    ≅〈 cong2 or (rename-subst-lem-p t) (rename-subst-lem-p u) 〉
      or (subst-p (subst-comp-ren f s) t) (subst-p (subst-comp-ren f s) u)
    ∎
  rename-subst-lem-p {Γ} {Γ'} {Γ''} {σ} {s} {f} (if b then t else u) = 
    ...
    ≅〈 cong2 (λ x y → if (⊢v-rename f (subst-v s b)) then x else y) (rename-subst-lem-p t) (rename-subst-lem-p u) 〉
      if (⊢v-rename f (subst-v s b)) then (subst-p (subst-comp-ren f s) t) else (subst-p (subst-comp-ren f s) u)
  ...
  subst-rename-lem-v {Γ} {Γ'} {Γ''} {unit} {s} {f} ⋆ = 
      ⋆
    ∎
  subst-rename-lem-v {Γ} {Γ'} {Γ''} {bit} {s} {f} zero = 
      zero
    ∎
  subst-rename-lem-v {Γ} {Γ'} {Γ''} {bit} {s} {f} one = 
      one
    ∎
  ...
  subst-comp-lem-v {Γ} {Γ'} {Γ''} {unit} {s} {s'} ⋆ = 
      ⋆
    ∎
  subst-comp-lem-v {Γ} {Γ'} {Γ''} {bit} {s} {s'} zero = 
      zero
    ∎
  subst-comp-lem-v {Γ} {Γ'} {Γ''} {bit} {s} {s'} one = 
      one
    ∎
  ...
  ≡-substcong-p {Γ} {Γ'} {σ} {s} (or-associativity {.Γ} {.σ} {t} {u} {v}) = 
    ...
    p∎
  ≡-substcong-p {Γ} {Γ'} {σ} {s} (if-zero {.Γ} {.σ} {t} {u}) = 
      if zero then (subst-p s t) else (subst-p s u)
    ≡p〈 if-zero 〉
      subst-p s t
    p∎
  ≡-substcong-p {Γ} {Γ'} {σ} {s} (if-one {.Γ} {.σ} {t} {u}) = 
      if one then (subst-p s t) else (subst-p s u)
    ≡p〈 if-one 〉
      subst-p s u
    p∎
  ...
  ≡-renamecong-p {Γ} {Γ'} {σ} {f} (or-associativity {.Γ} {.σ} {t} {u} {v}) = 
    ...
    p∎
  ≡-renamecong-p {Γ} {Γ'} {σ} {f} (if-zero {.Γ} {.σ} {t} {u}) = 
      if zero then (⊢p-rename f t) else (⊢p-rename f u)
    ≡p〈 if-zero 〉
      ⊢p-rename f t
    p∎
  ≡-renamecong-p {Γ} {Γ'} {σ} {f} (if-one {.Γ} {.σ} {t} {u}) = 
      if one then (⊢p-rename f t) else (⊢p-rename f u)
    ≡p〈 if-one 〉
      ⊢p-rename f u
    p∎
  ...
  ≡-renamecong-nv {Γ} {Γ'} {σ} {f} {.t} {.v} (≡-trans {.Γ} {.σ} {t} {u} {v} p q) = 
    ...
    ≡nv〈 ≡-renamecong-nv q 〉
      ⊢nv-rename f v
    nv∎
  ≡-renamecong-nv {Γ} {Γ'} {bit} {f} {bit2NV t} {bit2NV u} (congbit p) = 
      bit2NV (⊢av-rename f t)
    ≡nv〈 congbit (cong (⊢av-rename f) p) 〉
      bit2NV (⊢av-rename f u)
    nv∎
  ...
  ≡-renamecong-np {Γ} {Γ'} {σ} {f} (or-associativity {.Γ} {.σ} {t} {u} {v}) = 
    ...
    np∎
  ≡-renamecong-np {Γ} {Γ'} {σ} {f} (if-zero {.Γ} {.σ} {t} {u}) = 
      ifNP zeroNV then ⊢np-rename f t else (⊢np-rename f u)
    ≡np〈 if-zero 〉
      ⊢np-rename f t
    np∎
  ≡-renamecong-np {Γ} {Γ'} {σ} {f} (if-one {.Γ} {.σ} {t} {u}) = 
      ifNP oneNV then ⊢np-rename f t else (⊢np-rename f u)
    ≡np〈 if-one 〉
      ⊢np-rename f u
    np∎
  ...
  reflect-naturality-v {Γ} {Γ'} {bit} {f} (varAV x) = 
      bit2NV (varAV (f x))
    ∎
  ...
  reflect-naturality-v {Γ} {Γ'} {σ ⇀ τ} {f} (varAV x) = 
    ...
    ∎
  reflect-naturality-v {Γ} {Γ'} {bit} {f} (proj₁AV t) = 
      bit2NV (proj₁AV (⊢av-rename f t))
    ∎
  ...
  ⟦⟧v-naturality zero = refl
  ⟦⟧v-naturality one = refl
  ...
  *-rename-env-lem' {Γ} {Γ'} {Γ''} {σ} {σ'} {f} {e} u (T-if b d d') = 
    ...
    ∎
  *-rename-env-lem' {Γ} {Γ'} {Γ''} {σ} {σ'} {f} {e} u (T-input d) = 
      * {(Env-Denot Γ) ⊗ (Denot σ')} {Denot σ} (λ v → ⟦ u ⟧p (env-extend (fst v {_}) (snd v))) (t-r {Env-Denot Γ} {Denot σ'} {Γ''} ((λ {σ} x → e {σ} (f x)) , (T-input d)))
    ≅〈 cong T-input (*-rename-env-lem' {Γ} {Γ'} {Γ'' :: bit} {σ} {σ'} {f} {env-rename wk₁ e} u d) 〉
      * {(Env-Denot Γ') ⊗ (Denot σ')} {Denot σ}  (λ v → ⟦ ⊢p-rename (wk₂ f) u ⟧p (env-extend (fst v {_}) (snd v))) 
        (t-r {Env-Denot Γ'} {Denot σ'} {Γ''} ((λ {σ} → e {σ}) , (T-input d)))
    ∎
  *-rename-env-lem' {Γ} {Γ'} {Γ''} {σ} {σ'} {f} {e} u (T-output b d) = 
      * {(Env-Denot Γ) ⊗ (Denot σ')} {Denot σ} (λ v → ⟦ u ⟧p (env-extend (fst v {_}) (snd v))) (t-r {Env-Denot Γ} {Denot σ'} {Γ''} ((λ {σ} x → e {σ} (f x)) , (T-output b d)))
    ≅〈 cong (T-output b) (*-rename-env-lem' u d) 〉
      * {(Env-Denot Γ') ⊗ (Denot σ')} {Denot σ}  (λ v → ⟦ ⊢p-rename (wk₂ f) u ⟧p (env-extend (fst v {_}) (snd v))) 
        (t-r {Env-Denot Γ'} {Denot σ'} {Γ''} ((λ {σ} → e {σ}) , (T-output b d)))
    ∎
  ...
  rename-env-lem-v' {Γ} {Γ'} {Γ''} {σ ⇀ τ} {f} {e} (fn t) = 
    ...
    ∎
  rename-env-lem-v' zero = 
      zeroNV
    ∎
  rename-env-lem-v' one = 
      oneNV
    ∎
  ...
  rename-env-lem-p' {Γ} {Γ'} {Γ''} {σ} {f} {e} (output[ b , t ]) = 
    ...
    ≅〈 cong (λ x → T-output x(⟦ ⊢p-rename f t ⟧p e)) (rename-env-lem-v' {Γ} {Γ'} {Γ''} {bit} {f} {e} b) 〉
      T-output (⟦ ⊢v-rename f b ⟧v e) (⟦ ⊢p-rename f t ⟧p e)
    ∎
