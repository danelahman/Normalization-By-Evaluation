...
module Theory where
  ...
  data _⊢p_≡_ where
    ...
    state-ax1 : {Γ : Ctx} {σ : Ty} {t : Γ ⊢p σ} → Γ ⊢p lookup t t ≡ t
    state-ax2 : {Γ : Ctx} {σ : Ty} {t u t' u' : Γ ⊢p σ} → Γ ⊢p lookup (lookup t u) (lookup t' u') ≡ lookup t u'
    state-ax3 : {Γ : Ctx} {σ : Ty} {t u : Γ ⊢p σ} → Γ ⊢p update0 (lookup t u) ≡ update0 t
    state-ax4 : {Γ : Ctx} {σ : Ty} {t u : Γ ⊢p σ} → Γ ⊢p update1 (lookup t u) ≡ update1 u
  ...
  data _⊢np'_≡_ where
    ...
    -- congruence
    congupdate0 : {Γ : Ctx} {σ : Ty} {t t' : Γ ⊢np'' σ} → Γ ⊢np'' t ≡ t' → Γ ⊢np' update0NP t ≡ update0NP t'
    congupdate1 : {Γ : Ctx} {σ : Ty} {t t' : Γ ⊢np'' σ} → Γ ⊢np'' t ≡ t' → Γ ⊢np' update1NP t ≡ update1NP t'
  ...
  ⊢np-embed-lem {Γ} {σ} {.t} {.v} (≡-trans {.Γ} {.σ} {t} {u} {v} p q) = 
    ...
    ≡p〈 ⊢np-embed-lem q 〉
      ⊢np-embed v
    p∎
  ⊢np-embed-lem {Γ} {σ} {lookupNP t u} {lookupNP t' u'} (conglookup p q) = 
      lookup (⊢np'-embed t) (⊢np'-embed u)
    ≡p〈 conglookup (⊢np'-embed-lem p) (⊢np'-embed-lem q) 〉
      lookup (⊢np'-embed t') (⊢np'-embed u')
    p∎
  ...
  ⊢np'-embed-lem {Γ} {σ} {.t} {.v} (≡-trans {.Γ} {.σ} {t} {u} {v} p q) = 
    ...
    ≡p〈 ⊢np'-embed-lem q 〉
      ⊢np'-embed v
    p∎
  ⊢np'-embed-lem {Γ} {σ} {update0NP t} {update0NP t'} (congupdate0 p) = 
      update0 (⊢np''-embed t)
    ≡p〈 congupdate0 (⊢np''-embed-lem p) 〉
      update0 (⊢np''-embed t')
    p∎
  ⊢np'-embed-lem {Γ} {σ} {update1NP t} {update1NP t'} (congupdate1 p) = 
      update1 (⊢np''-embed t)
    ≡p〈 congupdate1 (⊢np''-embed-lem p) 〉
      update1 (⊢np''-embed t')
    p∎
