...
module PartialEquivalence where
  ...
  data _≈T_ {Γ : Ctx} {σ : Ty} {_≈_ : {Γ : Ctx} → ⟦ σ ⟧ Γ → ⟦ σ ⟧ Γ → Set} : T ⟦ σ ⟧ Γ → T ⟦ σ ⟧ Γ → Set where
    ...
    congif : {b b' : ⟦ bit ⟧ Γ} {d d' d'' d''' : T ⟦ σ ⟧ Γ} → Γ ⊢nv b ≡ b' → _≈T_ {Γ} {σ} {_≈_} d d' → _≈T_ {Γ} {σ} {_≈_} d'' d''' → _≈T_ {Γ} {σ} (T-if b d d'') (T-if b' d' d''')
    conginput : {d d' : T ⟦ σ ⟧ (Γ :: bit)} → _≈T_ {Γ :: bit} {σ} {_≈_} d d' → _≈T_ {Γ} {σ} (T-input d) (T-input d')
    congoutput : {b b' : ⟦ bit ⟧ Γ} {d d' : T ⟦ σ ⟧ Γ} → Γ ⊢nv b ≡ b' → _≈T_ {Γ} {σ} {_≈_} d d' → _≈T_ {Γ} {σ} (T-output b d) (T-output b' d')
    ...
    if-zero : {d d' d'' d''' : T ⟦ σ ⟧ Γ} → _≈T_ {Γ} {σ} {_≈_} d d'' → _≈T_ {Γ} {σ} {_≈_} d' d''' → _≈T_ {Γ} {σ} (T-if zeroNV d d') d''
    if-one : {d d' d'' d''' : T ⟦ σ ⟧ Γ} → _≈T_ {Γ} {σ} {_≈_} d' d'' → _≈T_ {Γ} {σ} {_≈_} d d''' → _≈T_ {Γ} {σ} (T-if oneNV d d') d''


  -- PER on semantic values
  _≈_ : {Γ : Ctx} {σ : Ty} → ⟦ σ ⟧ Γ → ⟦ σ ⟧ Γ → Set
  _≈_ {Γ} {bit} d d' = Γ ⊢nv d ≡ d'
  _≈_ {Γ} {unit} d d' = d ≅ d'
