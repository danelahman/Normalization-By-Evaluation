...
module Theory where
  ...
  data _⊢p_≡_ where
    ...
    congif : {Γ : Ctx} {σ : Ty} {b b' : Γ ⊢v Bool} {t u t' u' : Γ ⊢p σ} → Γ ⊢v b ≡ b' → Γ ⊢p t ≡ t' → Γ ⊢p u ≡ u' → Γ ⊢p if b then t else u ≡ if b' then t' else u'
    -- beta
    β⇀ : {Γ : Ctx} {σ τ : Ty} {t : Γ :: σ ⊢p τ} {u : Γ ⊢v σ} → Γ ⊢p subst-p (ext-subst id-subst u) t ≡ (app (lam t) u)
    ...
    ifto : {Γ : Ctx} {σ τ : Ty} {b : Γ ⊢v Bool} {t u : Γ ⊢p σ} {v : Γ :: σ ⊢p τ} → Γ ⊢p (if b then t else u) to v ≡ if b then (t to v) else (u to v)
    ...
    if-true : {Γ : Ctx} {σ : Ty} {t u : Γ ⊢p σ} → Γ ⊢p if true then t else u ≡ t
    if-false : {Γ : Ctx} {σ : Ty} {t u : Γ ⊢p σ} → Γ ⊢p if false then t else u ≡ u
  ...
  data _⊢nv_≡_ where
    ...
    -- congruence
    congav : {Γ : Ctx} {t u : Γ ⊢av α} → Γ ⊢av t ≡ u → Γ ⊢nv (av2NV t) ≡ (av2NV u)
    congpair : {Γ : Ctx} {σ₁ σ₂ : Ty} {t t' : Γ ⊢nv σ₁} {u u' : Γ ⊢nv σ₂} → Γ ⊢nv t ≡ t' → Γ ⊢nv u ≡ u' → Γ ⊢nv (pairNV t u) ≡ (pairNV t' u')
    conglam : {Γ : Ctx} {σ τ : Ty} {t u : Γ :: σ ⊢np τ} → Γ :: σ ⊢np t ≡ u → Γ ⊢nv lamNV t ≡ lamNV u
  ...
  data _⊢np_≡_ where
    ...
    congif : {Γ : Ctx} {σ : Ty} {b b' : Γ ⊢nv Bool} {t u t' u' : Γ ⊢np σ} → Γ ⊢nv b ≡ b' → Γ ⊢np t ≡ t' → Γ ⊢np u ≡ u' → Γ ⊢np ifNP b then t else u ≡ ifNP b' then t' else u'
    -- effect theory of nondeterministic choice
    ...
    -- effect theory of deterministic choice
    if-true : {Γ : Ctx} {σ : Ty} {t u : Γ ⊢np σ} → Γ ⊢np ifNP trueNV then t else u ≡ t
    if-false : {Γ : Ctx} {σ : Ty} {t u : Γ ⊢np σ} → Γ ⊢np ifNP falseNV then t else u ≡ u
  ...
  ⊢nv-embed-lem {Γ} {σ} {.t} {.v} (≡-trans {.Γ} {.σ} {t} {u} {v} p q) = 
    ...
    ≡v〈 ⊢nv-embed-lem q 〉
      ⊢nv-embed v 
    v∎
  ⊢nv-embed-lem {Γ} {α} {av2NV t} {av2NV .t} (congav refl) = 
      ⊢av-embed t
    ≡v〈 ≡-refl 〉
      ⊢av-embed t
    v∎
  ⊢nv-embed-lem {Γ} {σ₁ ∧ σ₂} {pairNV t u} {pairNV t' u'} (congpair p q) = 
    ...
    ≡v〈 congpair (⊢nv-embed-lem p) (⊢nv-embed-lem q) 〉
      pair (⊢nv-embed t') (⊢nv-embed u')
    v∎
  ⊢nv-embed-lem {Γ} {σ ⇀ τ} {lamNV t} {lamNV u} (conglam p) = 
      lam (⊢np-embed t)
    ≡v〈 conglam (⊢np-embed-lem p) 〉
      lam (⊢np-embed u)
    v∎
  ...
  ⊢np-embed-lem {Γ} {σ} {.(orNP (orNP t u) v)} {orNP t (orNP u v)}  or-associativity = 
    ...
    p∎
  ⊢np-embed-lem {Γ} {σ} {ifNP trueNV then t else u} {.t} if-true = 
      if true then ⊢np-embed t else (⊢np-embed u)
    ≡p〈 if-true 〉
      ⊢np-embed t
    p∎
  ⊢np-embed-lem {Γ} {σ} {ifNP falseNV then t else u} {.u} if-false = 
      if false then ⊢np-embed t else (⊢np-embed u)
    ≡p〈 if-false 〉
      ⊢np-embed u
    p∎
