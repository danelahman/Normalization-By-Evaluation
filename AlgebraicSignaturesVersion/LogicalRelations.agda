...
module LogicalRelations where
  ...
  ◁p-monotonicity {Γ} {Γ'} {σ} {f} {t} {T-input d d'} r = 
    (⊢p-rename f (fst r)) , 
  ...
  ◁p-monotonicity {Γ} {Γ'} {σ} {f} {t} {T-output0 d} r = 
    (⊢p-rename f (fst r)) , ((≡-renamecong-p (fst (snd r))) , (◁p-monotonicity {f = f} {t = fst r} {d = d} (snd (snd r))))
  ◁p-monotonicity {Γ} {Γ'} {σ} {f} {t} {T-output1 d} r = 
    (⊢p-rename f (fst r)) , ((≡-renamecong-p (fst (snd r))) , (◁p-monotonicity {f = f} {t = fst r} {d = d} (snd (snd r))))
  ...
  ◁p-invariance {Γ} {σ} {t} {t'} {T-to d d'} r p = 
    (fst r) , ((≡-trans (≡-sym p) (fst (snd r))) , (snd (snd r))) 
  ◁p-invariance {Γ} {σ} {t} {t'} {T-input d d'} r p = 
    (fst r) , ((fst (snd r)) , (((≡-trans (≡-sym p) (fst (fst (snd (snd r))))) , (snd (fst (snd (snd r))))) , (snd (snd (snd r)))))
  ◁p-invariance {Γ} {σ} {t} {t'} {T-output0 d} r p = 
    (fst r) , ((≡-trans (≡-sym p) (fst (snd r))) , (snd (snd r)))
  ◁p-invariance {Γ} {σ} {t} {t'} {T-output1 d} r p = 
    (fst r) , ((≡-trans (≡-sym p) (fst (snd r))) , (snd (snd r)))
  ...
  reify-p-lem t (T-input d d') r = 
    ≡-trans 
      (fst (fst (snd (snd r)))) 
      (conginput (reify-p-lem (fst r) d (snd (fst (snd (snd r))))) 
              (reify-p-lem (fst (snd r)) d' (snd (snd (snd r)))))
  reify-p-lem t (T-output0 d) r = 
    ≡-trans 
      (fst (snd r)) 
      (congoutput0 (reify-p-lem (fst r) d (snd (snd r))))
  reify-p-lem t (T-output1 d) r = 
    ≡-trans 
      (fst (snd r)) 
      (congoutput1 (reify-p-lem (fst r) d (snd (snd r))))
  ...
  ◁p-*-strength-fundamental-lemma {Γ} {Γ'} {σ} {τ} {s} {e} t (T-input d d') r u r' = 
    ((fst r) to (subst-p (lift s) u)) , 
    (((fst (snd r)) to (subst-p (lift s) u)) , 
      ((≡-trans 
         (congto (fst (fst (snd (snd r)))) ≡-refl) 
         inputto , 
      ◁p-*-strength-fundamental-lemma (fst r) d (snd (fst (snd (snd r)))) u r') , 
      ◁p-*-strength-fundamental-lemma (fst (snd r)) d' (snd (snd (snd r))) u r'))
  ◁p-*-strength-fundamental-lemma {Γ} {Γ'} {σ} {τ} {s} {e} t (T-output0 d) r u r' = 
    ((fst r) to (subst-p (lift s) u)) , ((≡-trans (congto (fst (snd r)) ≡-refl) output0to) , (◁p-*-strength-fundamental-lemma (fst r) d (snd (snd r)) u r'))
  ◁p-*-strength-fundamental-lemma {Γ} {Γ'} {σ} {τ} {s} {e} t (T-output1 d) r u r' = 
    ((fst r) to (subst-p (lift s) u)) , ((≡-trans (congto (fst (snd r)) ≡-refl) output1to) , (◁p-*-strength-fundamental-lemma (fst r) d (snd (snd r)) u r'))
